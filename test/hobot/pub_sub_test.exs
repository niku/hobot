defmodule Hobot.PubSubTest do
  ...
  defmodule CallbackSubscriber do
    ...
    def handle_cast(
          {:broadcast, topic, ref, data},
          {_registry, _topic, _before_receive, callback_pid} = state
        ) do
  ...
  setup context do
    ...
    {:ok, _} =
      Supervisor.start_link(
        [{Task.Supervisor, name: application_process.task_supervisor}],
  ...
  test "A subscriber receives a message which is published", %{
    application_process: application_process,
    callback_sup: callback_sup
  } do
    name = "Foo"
    ...
    {:ok, _subscriber} =
      Supervisor.start_child(callback_sup, [
        {application_process, topic, before_receive, callback_pid}
      ])
  ...
  test "A subscriber receives no message when it haven't subscribe the topic which is published",
       %{application_process: application_process, callback_sup: callback_sup} do
    name = "Foo"
    topic_value = "foo"
    topic = %Hobot.Topic{bot_name: name, value: topic_value}
    ...
    {:ok, _pid} =
      Hobot.PubSub.publish(application_process, no_subscribed_topic, ref, data, before_publish)

    refute_receive _anything
  end

  test "A subscriber receives a message even if other subscribers crashed", %{
    application_process: application_process,
    callback_sup: callback_sup,
    crash_sup: crash_sup
  } do
    ...
    {:ok, crashsubscriber} =
      Supervisor.start_child(crash_sup, [
        {application_process, topic, before_receive, callback_pid}
      ])
    ...
    receive do
      "terminated" ->
        ...
        Process.sleep(10)
        # assume the process is not alive.
        false = Process.alive?(crashsubscriber)
    end
  ...
  test "A subscriber receives message with low latency even if other subscribers are slow", %{
    application_process: application_process,
    callback_sup: callback_sup,
    slow_sup: slow_sup
  } do
    ...
    for _times <- 0..99 do
      {:ok, _slow_subscriber} =
        Supervisor.start_child(slow_sup, [
          {application_process, topic, before_receive, callback_pid}
        ])
    end

    {:ok, _callback_subscriber} =
      Supervisor.start_child(callback_sup, [
        {application_process, topic, before_receive, callback_pid}
      ])
    ...
    for _times <- 0..2 do
      ...
      {:ok, _pid} = Hobot.PubSub.publish(application_process, topic, ref, data, before_publish)
      # timeout 100ms
      assert_receive {:broadcast, ^topic_value, ^ref, ^data}, 100
    end
  end

  test "The broker eliminates crashed subscriber from own registry", %{
    application_process: application_process,
    callback_sup: callback_sup
  } do
    # See also
