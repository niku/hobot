defmodule Hobot.Bot do
  ...
  def middleware(atom, adapter, handlers) do
    for {handler_conf, index} <- handlers,
        into: %{adapter(atom) => Hobot.Middleware.build(adapter)} do
      {handler(atom, index), Hobot.Middleware.build(handler_conf)}
    end
  end

  def subscribe(
        %Hobot.ApplicationProcess{} = application_process,
        %Hobot.Topic{} = topic,
        middleware
      ) do
    ...
    before_receive =
      get_in(middleware, [Access.key!(registered_name), Access.key!(:before_receive)])
  ...
  def publish(
    ...
    before_publish =
      get_in(middleware, [Access.key!(registered_name), Access.key!(:before_publish)])
  ...
  def reply(
    ...
    Task.Supervisor.start_child(application_process.task_supervisor, fn ->
      case Hobot.Middleware.apply_middleware(before_reply, reply) do
        {:ok, value} ->
          GenServer.cast(
            {:via, Registry, {application_process.name_registry, adapter_name}},
            value
          )

        {:halt, value} ->
          application_process.logger.bare_log(
            :debug,
            "halted at before reply. reason: #{inspect(value)}"
          )
      end
    end)
  end
  ...
  def make_context(name, adapter, handlers, %Hobot.ApplicationProcess{} = application_process) do
    ...
    %{
      ...
      subscribe:
        &subscribe(application_process, %Hobot.Topic{bot_name: name, value: &1}, middleware),
      publish:
        &publish(application_process, %Hobot.Topic{bot_name: name, value: &1}, &2, &3, middleware),
